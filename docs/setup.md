...
The stack is two containers sharing one data volume:
...
| **control** | A small Node/Hono service: the only writer of the shared content tree and the credentials file. It mints hosts (`create`) and changes/removes them (`rotate`, `remove`). |

Both speak **plain HTTP**. TLS and routing are provided by a reverse proxy you put in
front (see [Ingress](#ingress)) — that part is deliberately out of scope and reusable.
...
This builds the control image, pulls Apache, and starts both services bound to the address
in your `.env`.
...
| `PLANDROP_CONTROL_PORT` | Plain-HTTP port for the control plane. | `8081` |
...
|----------|----------|---------|
| `plandrop.example.com` (apex) | `http://<PLANDROP_BIND>:<PLANDROP_CONTROL_PORT>` | control plane (create / rotate / remove) |
| `*.plandrop.example.com` | `http://<PLANDROP_BIND>:<PLANDROP_APACHE_PORT>` | static + WebDAV serving |
...
> ⚠️ **Security:** `create` has **no authentication** — anyone who can reach the control
> plane can mint a host. Restrict who can reach the apex route (LAN-only, VPN, or a proxy
> access list). Per-host writes are always authenticated by the generated passphrase; reads
...
```sh
BIND=172.17.0.1; A=8080; C=8081; D=plandrop.example.com
# create
read host pass < <(curl -s -X POST http://$BIND:$C/api/hosts \
...
curl -s -o /dev/null -w '%{http_code}\n' -X DELETE -u "$host:$pass" \
  http://$BIND:$C/api/hosts/$host                                          # 204
```
